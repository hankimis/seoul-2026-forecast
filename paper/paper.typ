...
#align(center)[
  ...
  #box(stroke: 0.6pt + luma(180), inset: (x: 9pt, y: 4pt), radius: 3pt)[
    #text(8.5pt, fill: luma(90))[Pre-registered working paper — predictions committed before the outcome, graded after polls close on 2026-06-03]
  ]
]
...
#figure(image("figs/results_tilemap.png", width: 100%), caption: [Predicted (left) versus actual (right). Blue is the Democratic Party, red is the People Power Party. Only 서울 and 경남, marked with a heavy border, flipped against the forecast; the model called the other fourteen correctly.])
...
#figure(image("figs/results_scatter.png", width: 72%), caption: [Predicted versus actual two-way Democratic share. Points on the dashed diagonal are perfect; the green cluster (hits) sits close to it, while the two red points (서울, 경남) straddle the 50% line, which is precisely where a sub-three-point error flips a winner.])
...
#figure(
  table(
    ...
    [2026-06-03 18:00 (v2.0)], [Polls close. Enter realized winners and shares; run the scoring script. Report winner accuracy (/16), two-way MAE, total-votes error, and the realized Brier @eq-brier against the $plus.minus 3%$ target. Publish the result; write the verdict.],
  ),
